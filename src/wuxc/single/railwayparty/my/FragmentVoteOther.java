...
import wuxc.single.railwayparty.adapter.VoteOtherAdapter;
import wuxc.single.railwayparty.internet.HttpGetData;
import wuxc.single.railwayparty.model.Bbs1Model;
import wuxc.single.railwayparty.model.VoteOtherModel;

public class FragmentVoteOther extends Fragment implements OnTouchListener, OnClickListener, OnItemClickListener {
	private ListView ListData;
	List<VoteOtherModel> list = new ArrayList<VoteOtherModel>();
	private static VoteOtherAdapter mAdapter;
...
//				Toast.makeText(getActivity(), "数据格式校验失败", Toast.LENGTH_SHORT).show();
		...
		try {
			...
			if (jArray.length() == 0) {
				...
				for (int i = 0; i < jArray.length(); i++) {
					...
					// json_data =
					// json_data.getJSONObject(json_data.getString("coverImage"));
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (null != view) {
			...
			view = inflater.inflate(R.layout.wuxc_fragment_vote_other, container, false);
			initview(view);
			setonclicklistener();
			setheadtextview();
	...
	private void GetData() {
		...
		ArrayValues.add(new BasicNameValuePair("classify", "matterVote"));
		// chn = "dyq";''
		ArrayValues.add(new BasicNameValuePair("curPage", "" + curPage));
		ArrayValues.add(new BasicNameValuePair("pageSize", "" + pageSize));
	...
	protected void go() {
		ListData.setPadding(0, -100, 0, 0);
		mAdapter = new VoteOtherAdapter(getActivity(), list, ListData);
