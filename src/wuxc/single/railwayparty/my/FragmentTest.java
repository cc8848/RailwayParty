...
import wuxc.single.railwayparty.adapter.TestAdapter.Callback;
import wuxc.single.railwayparty.adapter.TestAdapter;
import wuxc.single.railwayparty.internet.HttpGetData;
import wuxc.single.railwayparty.main.ExamResultActivity;
...
public class FragmentTest extends Fragment implements OnTouchListener, Callback, OnClickListener, OnItemClickListener {
		...
		try {
			...
			if (jArray.length() == 0) {
				...
				for (int i = 0; i < jArray.length(); i++) {
					...
					listinfo.setTime(json_data.getString("createTime"));
					listinfo.setTitle(json_data.getString("paperTitle"));
					listinfo.setImageUrl("");

					listinfo.setDetail("" + json_data.getInt("score"));
					list.add(listinfo);
	...
	public void click(View v) {
		...
		switch (v.getId()) {
		case R.id.lin_all:
			TestModel data = list.get((Integer) v.getTag());
			...
			intent.setClass(getActivity(), ExamResultActivity.class);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (null != view) {
			...
			view = inflater.inflate(R.layout.wuxc_fragment_test, container, false);
			initview(view);
			setonclicklistener();
			setheadtextview();
	...
	protected void go() {
		ListData.setPadding(0, -100, 0, 0);
		mAdapter = new TestAdapter(getActivity(), list, ListData, this);
