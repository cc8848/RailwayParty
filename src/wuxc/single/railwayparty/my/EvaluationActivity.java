...
import wuxc.single.railwayparty.adapter.EvaluationAdapter;
...
public class EvaluationActivity extends Activity implements OnClickListener, OnTouchListener, OnItemClickListener {
	private ListView ListData;
	List<EvaluationModel> list = new ArrayList<EvaluationModel>();
	private static EvaluationAdapter mAdapter;
		...
		try {
			...
			if (jArray.length() == 0) {
				...
				for (int i = 0; i < jArray.length(); i++) {
					...
					EvaluationModel listinfo = new EvaluationModel();

					// listinfo.setTime(json_data.getString("operateTime"));
					// listinfo.setTitle(json_data.getString("ctype"));
					// //
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.wuxc_activity_evaluation);
	...
	private void getdatalist(int arg) {
		...
		try {
			...
			for (int i = 0; i < 10; i++) {
				...
				listinfo.setDetail("作为一名单元，让我懂得了为人民服务！");
				list.add(listinfo);
	...
	protected void go() {
		ListData.setPadding(0, -100, 0, 0);
		mAdapter = new EvaluationAdapter(this, list, ListData);
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		// TODO Auto-generated method stub
//		EvaluationModel data = list.get(position - 1);
//		if (data.isCont()) {
...
//		}
	...
	private void GetData() {
		...
		new Thread(new Runnable() { // 开启线程上传文件
			...
			public void run() {
				String DueData = "";
				DueData = HttpGetData.GetData("api/pb/partyEvaluation/getListJsonData", ArrayValues);
