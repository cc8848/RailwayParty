...
public class BuildFragment2 extends Fragment
	...
	private SharedPreferences PreForDJYW;
			...
			if (Type.equals(GET_SUCCESS_RESULT)) {
				...
				try {
					...
					if (jArray.length() == 0) {
						...
						Log.e("DJYWread", "" + (PreForDJYW.getAll().size() - 1));
						edit1.commit();
		...
		try {
			...
			if (jArray.length() == 0) {
				...
				for (int i = 0; i < jArray.length(); i++) {
					...
					listinfo.setRead(PreForDJYW.getBoolean(json_data.getString("keyid"), false));
					try {
	...
	private void GetPager(String pager) {
		...
		try {
			...
			edit.putInt("DJYWtotal", totalcount);
			edit.commit();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (null != view) {
			...
			PreForDJYW = getActivity().getSharedPreferences("DJYW", Context.MODE_PRIVATE);
			ItemNumber = getActivity().getSharedPreferences("ItemNumber", Context.MODE_PRIVATE);

			ReadTicket();
			GetData();
			if (!PreForDJYW.getBoolean("DJYW", false)) {
				GetMyReadRecord();
			}
		}
	...
	private void GetMyReadRecord() {
		...
		ArrayValues.add(new BasicNameValuePair("accessRecordDto.classify", "djyw"));
	...
	protected void go() {
		ListData.setPadding(0, -100, 0, 0);
		mAdapter = new BuildAdapter2(getActivity(), list, ListData, this);
		ListData.setAdapter(mAdapter);
//		Editor edit = PreForDJYW.edit();
//		edit.clear();
//		edit.commit();
//		Editor edit2 = PreForDJYW.edit();
//		edit2.putBoolean("DJYW", true);
