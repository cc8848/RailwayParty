...
public class FlagFragment3 extends Fragment implements Callback, OnTouchListener, OnClickListener, OnItemClickListener {
	...
	private static Flag3Adapter mAdapter;
	...
	private SharedPreferences PreForQNXF;
			...
			if (Type.equals(GET_SUCCESS_RESULT)) {
				...
				try {
					...
					if (jArray.length() == 0) {
						...
						Log.e("QNXFread", "" + (PreForQNXF.getAll().size() - 1));
						edit1.commit();
					}
		...
		try {
			...
			if (jArray.length() == 0) {
				...
				for (int i = 0; i < jArray.length(); i++) {
					...
					Flag3Model listinfo = new Flag3Model();
					...
					listinfo.setRead(PreForQNXF.getBoolean(json_data.getString("keyid"), false));
	...
	private void GetPager(String pager) {
		...
		try {
			...
			edit.putInt("QNXFtotal", totalcount);
			edit.commit();
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (null != view) {
			...
			PreForQNXF = getActivity().getSharedPreferences("QNXF", Context.MODE_PRIVATE);
			ItemNumber = getActivity().getSharedPreferences("ItemNumber", Context.MODE_PRIVATE);

			ReadTicket();

			GetData();
			if (!PreForQNXF.getBoolean("QNXF", false)) {
				GetMyReadRecord();
			}
		}
	...
	private void GetMyReadRecord() {
		...
		ArrayValues.add(new BasicNameValuePair("accessRecordDto.classify", "qnxf"));
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		// TODO Auto-generated method stub
		Flag3Model data = list.get(position - 1);
		if (true) {
	...
	private void GetData() {
		...
		final ArrayList ArrayValues = new ArrayList();
		ArrayValues.add(new BasicNameValuePair("ticket", "" + ticket));
		ArrayValues.add(new BasicNameValuePair("chn", "qnxf"));
		chn = "qnxf";
