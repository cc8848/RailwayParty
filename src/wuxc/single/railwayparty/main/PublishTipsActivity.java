...
public class PublishTipsActivity extends FragmentActivity implements OnClickListener {
	...
	protected void GetDataDueData(Object obj) {
		...
		try {
			...
			if (Type.equals(GET_SUCCESS_RESULT)) {
				...
				ArrayValues.add(new BasicNameValuePair("userScoreDto.reason", "发表学习心得"));
		...
		gridView.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
				if (position == (photoList.size())) {
					menuWindow = new SelectPicPopupWindow(PublishTipsActivity.this, itemsOnClick);
					menuWindow.showAtLocation(PublishTipsActivity.this.findViewById(R.id.uploadPictureLayout),
							Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0); // 设置layout在PopupWindow中显示的位置
				} else {
					Intent intent = new Intent(PublishTipsActivity.this, ViewPagerDeleteActivity.class);
		...
		public void onClick(View v) {
			...
			case R.id.btn_take_photo: {
				...
				if (status.equals(Environment.MEDIA_MOUNTED)) {
					...
					Toast.makeText(PublishTipsActivity.this, "没有SD卡", Toast.LENGTH_LONG).show();
				}
				break;
			}
	...
	protected void doPickPhotoFromGallery() {
		try {
			...
			final Intent intent = new Intent(PublishTipsActivity.this, PhotoAlbumActivity.class);
