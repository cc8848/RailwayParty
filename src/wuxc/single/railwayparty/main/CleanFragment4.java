...
public class CleanFragment4 extends Fragment
	...
	private static Clean4Adapter mAdapter;
	...
	private SharedPreferences PreForXXJL;
			...
			if (Type.equals(GET_SUCCESS_RESULT)) {
				...
				try {
					...
					if (jArray.length() == 0) {
						...
						Log.e("XXJLread", "" + (PreForXXJL.getAll().size() - 1));
						edit1.commit();
					}
		...
		try {
			...
			if (jArray.length() == 0) {
				...
				for (int i = 0; i < jArray.length(); i++) {
					...
					listinfo.setRead(PreForXXJL.getBoolean(json_data.getString("keyid"), false));
	...
	private void GetPager(String pager) {
		...
		try {
			...
			edit.putInt("XXJLtotal", totalcount);
			edit.commit();
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (null != view) {
			...
			PreForXXJL = getActivity().getSharedPreferences("XXJL", Context.MODE_PRIVATE);
			ItemNumber = getActivity().getSharedPreferences("ItemNumber", Context.MODE_PRIVATE);

			ReadTicket();

			GetData();
			if (!PreForXXJL.getBoolean("XXJL", false)) {
				GetMyReadRecord();
			}
		}
	...
	private void GetMyReadRecord() {
		...
		ArrayValues.add(new BasicNameValuePair("accessRecordDto.classify", "xxjl"));
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		// TODO Auto-generated method stub
		Clean4Model data = list.get(position - 1);
		if (true) {
...
//		edit.clear();
//		edit.commit();
//		Editor edit2 = PreForXXJL.edit();
//		edit2.putBoolean("XXJL", true);
//		for (int i = 0; i < list.size(); i++) {
//			Clean4Model info = list.get(i);
//			if (info.isRead()) {
