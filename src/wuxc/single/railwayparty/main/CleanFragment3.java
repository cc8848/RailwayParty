...
public class CleanFragment3 extends Fragment
	...
	private static Clean3Adapter mAdapter;
	...
	private SharedPreferences PreForQLKM;
			...
			if (Type.equals(GET_SUCCESS_RESULT)) {
				...
				try {
					...
					if (jArray.length() == 0) {
						...
						Log.e("QLKMread", "" + (PreForQLKM.getAll().size() - 1));
						edit1.commit();
					}
		...
		try {
			...
			if (jArray.length() == 0) {
				...
				for (int i = 0; i < jArray.length(); i++) {
					...
					Clean3Model listinfo = new Clean3Model();
	...
	private void GetPager(String pager) {
		...
		try {
			...
			edit.putInt("QLKMtotal", totalcount);
			edit.commit();
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (null != view) {
			...
			PreForQLKM = getActivity().getSharedPreferences("QLKM", Context.MODE_PRIVATE);
			ItemNumber = getActivity().getSharedPreferences("ItemNumber", Context.MODE_PRIVATE);

			ReadTicket();

			GetData();
			if (!PreForQLKM.getBoolean("QLKM", false)) {
				GetMyReadRecord();
			}
		}
	...
	private void GetMyReadRecord() {
		...
		ArrayValues.add(new BasicNameValuePair("accessRecordDto.classify", "qlkm"));
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		// TODO Auto-generated method stub
		Clean3Model data = list.get(position - 1);
		if (true) {
	...
	public void click(View v) {
		...
		switch (v.getId()) {
		case R.id.lin_all:
			Clean3Model data = list.get((Integer) v.getTag());
			if (true) {
				Intent intent = new Intent();
				intent.setClass(getActivity(), SpecialDetailActivity.class);
				Bundle bundle = new Bundle();
	...
	private void GetData() {
		...
		ArrayValues.add(new BasicNameValuePair("chn", "qlkm"));
		chn = "qlkm";
		ArrayValues.add(new BasicNameValuePair("curPage", "" + curPage));
		ArrayValues.add(new BasicNameValuePair("pageSize", "" + pageSize));
