...
public class MemberFragment1 extends Fragment
			...
			if (Type.equals(GET_SUCCESS_RESULT)) {
				...
				try {
					...
					if (jArray.length() == 0) {
						...
						Log.e("JSZNread", "" + (PreForJSZN.getAll().size() - 1));
						edit1.commit();
					}
	...
	private void GetPager(String pager) {
		...
		try {
			...
			edit.putInt("JSZNtotal", totalcount);
			edit.commit();
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (null != view) {
			...
			PreForJSZN = getActivity().getSharedPreferences("JSZN", Context.MODE_PRIVATE);
			ItemNumber = getActivity().getSharedPreferences("ItemNumber", Context.MODE_PRIVATE);

			ReadTicket();

			GetData();
			if (!PreForJSZN.getBoolean("JSZN", false)) {
				GetMyReadRecord();
			}
		}
	...
	private void GetMyReadRecord() {
		...
		ArrayValues.add(new BasicNameValuePair("accessRecordDto.classify", "jszn"));
	...
	private void GetData() {
		...
		final ArrayList ArrayValues = new ArrayList();
		ArrayValues.add(new BasicNameValuePair("ticket", "" + ticket));
		ArrayValues.add(new BasicNameValuePair("chn", "jszn"));
		chn = "jszn";
